...
INSERT overwrite TABLE knowyou_ott_ods.odm_tair_video_dm partition(dt,ht)
SELECT  
    a.deviceid,
    get_json_object(j.info,'$.id') AS id,
    get_json_object(j.info,'$.deviceType') AS device_type,
...
select
 concat('update sys_device set province="',province,'", city="',city,'" where deviceid="',deviceid,'";')
from
(
    select
       deviceid,
       province,
       city,
       row_number() over(partition by deviceid oder by dt) rn
    from knowyou_ott_ods.odm_tair_ping_dm
    where length(province)>0 and length(city)>0 and length(deviceid)>0  
) t
where rn = 1