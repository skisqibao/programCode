 ...
 * 付费评估
 */
public class PayPlusRecommend {
    private static final Logger log = LoggerFactory.getLogger(PayPlusRecommend.class);
    //决定是否手动选择日期
    ...
    public static void main(String[] args) {
        ...
        if (ALS_PAY_DATA_AUTO) {
            ...
            ALS_PAY_ORDER_DATA_TIME = config.getProperty(UtilConstants.PayAlsRec.ALS_PAY_ORDER_DATA_TIME, "202011");
        }
...
//            List<Long> valDistinctUserList = valDistinctUserRdd.map(x -> Long.parseLong(String.valueOf(x))).collect();
            ...
            String insertTime = ALS_PAY_ORDER_DATA_TIME;
