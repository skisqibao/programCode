...
public interface UserRepository extends Neo4jRepository<MediaNode, Long> {
   ...
   @Query("MERGE (m:Media {videoId: {videoId}, videoName: {videoName},videoScore:{videoScore},videoRegion:{videoRegion},videoType:{videoType},cpType:{cpType}}) with m MERGE(d:Director {directName:{directName}}) MERGE (m) <-[:DIRECTED]-(d) with m MERGE(a:Actor {actorName: {actorName}}) MERGE (m)<-[:ACTED_IN]-(a) with m MERGE(t:Plot {videoPlot: {videoPlot}}) MERGE (m) -[:IN_GENRE]-> (t) with m MERGE(r:Region {videoRegion: {videoRegion}}) MERGE (m) -[:IN_REGION]-> (r)")
   void addMediaNodeList(@Param("videoId") String videoId, @Param("videoName") String videoName, @Param("videoScore") String videoScore, @Param("videoRegion") String videoRegion, @Param("videoType") String videoType, @Param("directName") String directName, @Param("actorName") String actorName, @Param("videoPlot") String videoPlot, @Param("cpType") String cpType);
