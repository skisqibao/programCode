...
object PaidPromotionAdjustParameter {
...
//    transformationRdd(spark)
  ...
  def handleVideoData(spark: SparkSession): Unit = {
    ...
    sampleDeviceRdd.coalesce(6).saveAsTextFile("/sk/chongqing/handle_data/device_video_data/")
  }
  ...
  def trainAndPredict(spark: SparkSession): Unit = {
    ...
    val rating = spark.sparkContext.textFile("/sk/chongqing/model/input/rating/")
    val ratingRdd = rating.map(x => Rating(x.split(",")(0).toInt, x.split(",")(1).toInt, 1.0))
    ...
    val finalValidationSetRdd = spark.sparkContext.textFile("/sk/chongqing/handle_data/set-validation-device-production-data/")
    val validationSetRdd = finalValidationSetRdd.map(x => (x.split(",")(0), x.split(",")(1)))

    // 获取验证集产品包index
    val validationProductionIndexArray = validationSetRdd.map(_.swap).join(productionWithIndexRdd)
      .map(_._2._2).distinct().collect

    logger.info("验证集产品包数量：" + validationProductionIndexArray.length)

    var sumF1score = 0.0
    var cnt = 0

    val deviceWithIndex = spark.sparkContext.textFile("/sk/chongqing/model/input/deviceWithIndex/")
    val deviceWithIndexRdd = deviceWithIndex.map(x => (x.split(",")(0), x.split(",")(1).toInt))
    val validationProductionListRdd = spark.sparkContext.textFile("/sk/chongqing/list/validation-production/")
    val validationProductionRdd = validationProductionListRdd.map(x => (x.split("#")(0), x.split("#")(1)))

    for (productionIndex <- validationProductionIndexArray) {
      // 对每个产品包推荐用户，取topN个, N取值借鉴验证集每个产品包订购用户的平均值 ValidationListSize
      val ratings = model.recommendUsers(productionIndex, 86)
      ...
      // 当前产品包
      val productionName = predictResultWithIdRdd.map(_._2).take(1).head

      // 当前产品包下 预测订购用户列表
      val predictResultDeviceList = predictResultWithIdRdd.map(_._1).collect
      val predictLength = predictResultDeviceList.length

//      logger.info("预测结果列表: " + predictResultDeviceList.foreach(println))

      // 当前产品包下 实际订购用户列表
      val actualResultDeviceList = validationProductionRdd.filter(_._1.equals(productionName)).flatMap(_._2.split(","))
      val actualLength = actualResultDeviceList.count

//      logger.info("实际订购列表: " + actualResultDeviceList.foreach(println))

      // 命中用户数
      val hit = actualResultDeviceList.filter(x => predictResultDeviceList.contains(x)).count.toDouble

      // 当前用户f1-score
      sumF1score += (2 * hit) / (predictLength + actualLength)
      cnt += 1

      logger.info("产品包Index: " + productionIndex + "\t产品包Name: " + productionName + "\n"
        + "预测订购列表用户数: " + predictLength + "\t实际订购列表用户数: " + actualLength + "\n"
        + "预测命中数：" + hit + "\n"
        + "当前f1-score累加值: " + sumF1score + "\t当前包数：" + cnt)

    }

    val meanF1score = sumF1score / cnt

    logger.info("最终f1-score平均值\tALS_RANK\tALS_NUM_ITERATIONS\tALS_LAMBDA\tALS_ALPHA\n"
      + meanF1score + "\t" + ALS_RANK + "\t" + ALS_NUM_ITERATIONS + "\t" + ALS_LAMBDA + "\t" + ALS_ALPHA)
